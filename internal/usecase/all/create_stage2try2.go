...
func (usecase *allUsecaseImpl) CreateStage2try2(req model.ByIdRequest) model.WebServiceResponse {
	...
	if exists {
		return util.ToWebServiceResponse("Hasil ujian stage 2 percobaan 2 sudah ada", http.StatusConflict, nil)
	}

	stage2, err := usecase.Store.CreateStage2try2(context.Background(), req.ID)
	if err != nil {
		return util.ToWebServiceResponse("Gagal membuat hasil ujian stage 2 percobaan 2: "+err.Error(), http.StatusInternalServerError, nil)
	}

	return util.ToWebServiceResponse("Berhasil membuat hasil ujian stage 2 percobaan 2", http.StatusCreated, gin.H{
		"stage_2": model.CreateStage2try2{
