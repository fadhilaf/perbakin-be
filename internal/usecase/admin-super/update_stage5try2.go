...
func (usecase *adminSuperUsecaseImpl) UpdateStage5try2(req model.UpdateStage5try2Request) model.WebServiceResponse {
	newStage5, err := usecase.Store.UpdateStage5try2(context.Background(), repositoryModel.UpdateStage5try2Params{
		ID:             req.ID,
		...
		Try2Status:     repositoryModel.Stage5Status(req.Try2.Status),
	...
	return util.ToWebServiceResponse("Berhasil mengubah hasil ujian stage 5", http.StatusOK, gin.H{
		"stage_5": model.UpdateStage5try2Response{
			...
			Try2: model.Stage5Try{
				Status:     string(newStage5.Try2Status),
				No1:        util.Stage5DatabaseNumbersToStruct(newStage5.Try2No1),
				No2:        util.Stage5DatabaseNumbersToStruct(newStage5.Try2No2),
				Checkmarks: util.CheckmarksToBoolArray(newStage5.Try2Checkmarks),
			},
