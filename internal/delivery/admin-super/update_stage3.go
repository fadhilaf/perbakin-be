...
func (handler *adminSuperHandler) UpdateStage3(c *gin.Context) {
	...
	if stage3.IsTry2 {
		...
		//check series try2
		if ok := util.CheckNumbers(c, model.Stage3Type,
			req.Try1.No1.Scores,
			req.Try1.No2.Scores,
			req.Try1.No3.Scores,
			req.Try1.No4.Scores,
		...
		if ok := util.CheckCheckmarksStage123456(c, req.Try1.Checkmarks, model.Stage3Type); !ok {
			return
		}
		//check checkmarks try 2
		if ok := util.CheckCheckmarksStage123456(c, req.Try2.Checkmarks, model.Stage3Type); !ok {
			return
		}

		res := handler.Usecase.UpdateStage3try2(model.UpdateStage13try2Request{
			ID: stage3.ID,
		...
		res := handler.Usecase.UpdateStage3try1(model.UpdateStage13try1Request{
			ID: stage3.ID,
			Try1: model.Stage13TryString{
				Status:     req.Try1.Status,
				No1:        util.IntArraysToScoresAndDuration(req.Try1.No1.Scores, req.Try1.No1.Duration),
