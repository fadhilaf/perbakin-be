...
func (handler *adminSuperHandler) DeleteStage4(c *gin.Context) {
	...
	if result.Stage == string(model.Stage4TypeString) {
		if err := handler.Usecase.UpdateResultStage(model.UpdateResultStageRequest{
			ID:    result.ID,
			Stage: string(model.Stage3TypeString),
		}); err {
			resMsg := util.ToWebServiceResponse("Terjadi kesalahan ketika memundurkan ke stage 3", http.StatusInternalServerError, nil)
