...
func (handler *adminSuperHandler) UpdateStage4(c *gin.Context) {
	...
	if stage4.IsTry2 {
		...
		//check series try2
		if ok := util.CheckNumbers(c, model.Stage4Type,
		...
		if ok := util.CheckCheckmarksStage123456(c, req.Try1.Checkmarks, model.Stage4Type); !ok {
			return
		}
		//check checkmarks try 2
		if ok := util.CheckCheckmarksStage123456(c, req.Try2.Checkmarks, model.Stage4Type); !ok {
			return
		}

		res := handler.Usecase.UpdateStage4try2(model.UpdateStage246try2Request{
			ID: stage4.ID,
		...
		res := handler.Usecase.UpdateStage4try1(model.UpdateStage246try1Request{
			ID: stage4.ID,
			Try1: model.Stage246TryString{
				Status:     req.Try1.Status,
				No1:        util.IntArraysToScoresAndDuration(req.Try1.No1.ScoresA, req.Try1.No1.ScoresB, req.Try1.No1.Duration),
